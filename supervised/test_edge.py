...
@torch.no_grad()
def confusion_matrix_for_paper(model, data, paper_id, target_edge_type, device, threshold=0.5):
    model.eval()
    ...
    # -----------------------------
    # 2. Predict probabilities for ALL fields
    # -----------------------------
    paper_rep = paper_emb.unsqueeze(0).repeat(num_fields, 1)
    logits = model.edge_classifier(paper_rep, field_emb)
    probs = torch.sigmoid(logits)

    pred_mask = (probs > threshold).cpu().numpy()  # predicted positives

    # -----------------------------
    # 3. Get TRUE labels for ALL fields
    # -----------------------------
...
@torch.no_grad()
def visualize_paper_prediction(model, full_data, paper_id, target_edge_type, device, threshold=0.5):

    model.eval()
    src_type, _, dst_type = target_edge_type

    # --------------------------------------------------------
    # 1. SAMPLE A SUBGRAPH AROUND THE PAPER
    # --------------------------------------------------------
    ...
    # Compute scores for paper → every field in batch
    paper_rep = paper_emb.unsqueeze(0).repeat(num_fields, 1)
    logits = model.edge_classifier(paper_rep, field_emb)
    probs = torch.sigmoid(logits).cpu().numpy()

    # Predictions: convert LOCAL batch indices → GLOBAL field IDs
    predicted_local = np.where(probs > threshold)[0]
    predicted_global = batch_field_global_ids[predicted_local]

    # --------------------------------------------------------
    # 3. GET TRUE FIELD EDGES FROM THE FULL GRAPH
    # --------------------------------------------------------
    full_edge_index = full_data[target_edge_type].edge_index.cpu()

    true_mask_global = (full_edge_index[0] == paper_id)
    true_fields_global = full_edge_index[1][true_mask_global].cpu().numpy()

    # --------------------------------------------------------
    # 4. Build Visualization Graph
    # --------------------------------------------------------
    G = nx.Graph()
    ...
    plt.savefig("plots/paper_graph_prediction.png", dpi=300)
    plt.close()

    print("Saved visualization to plots/paper_graph_prediction.png")
...
if __name__ == "__main__":
    ...
    # --------------------------------------------------------
    # Build evaluation pipeline wrapper
    # --------------------------------------------------------
